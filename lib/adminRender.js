...
const ADMIN_STYLES = `
  ...
  .item-card .fields select {
    font-size: 0.88rem; padding: 9px 10px; border-radius: 10px;
    border: 1px solid var(--card-border); background: var(--bg); color: var(--fg); font-family: inherit;
  }
...
<body>
  <div class="page admin">
    ...
    <fieldset>
      <legend>Campaign tracking (optional)</legend>
      <div class="hint" style="margin: 0 0 12px;">Appended as UTM query params to every outbound link click, so you can see traffic sources in your analytics. Leave blank to skip.</div>

      <label for="f-utm-source">utm_source</label>
      <input type="text" id="f-utm-source" maxlength="60" placeholder="e.g. linktree, bio, instagram" />

      <label for="f-utm-medium">utm_medium</label>
      <input type="text" id="f-utm-medium" maxlength="60" placeholder="e.g. social, link-in-bio" />

      <label for="f-utm-campaign">utm_campaign</label>
      <input type="text" id="f-utm-campaign" maxlength="60" placeholder="e.g. summer-2026" />
    </fieldset>
      ...
      if (item.type === 'video') {
        ...
        card.innerHTML = \`
          ...
          <div class="fields">
            ...
            <select class="full" data-field="style">
              <option value="classic"\${style === 'classic' ? ' selected' : ''}>Classic (stacked row)</option>
              <option value="featured"\${style === 'featured' ? ' selected' : ''}>Featured (large 16:9 card, needs an image)</option>
            </select>
            <div class="thumb-row">
